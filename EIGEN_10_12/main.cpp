...
void RLmult(double* a, int n, int k1, int k2, double *RL_save);
int LRdeform(double* a, int n, int k1, int k2, double Norma);
double Trace(double* a, int n);
void EigenValImpl(double* a, int size, double* eig, int begin, int end, double Norma, double* save, double *RL_save);
...
void pickUpGoodSk(double& s_k, double* a, int n, int begin, int end, double Norma, double* save) {
	...
	double val11 = 0, val12 = 0 , temp_val=0;
	EigValDim2Sdvig(a, n, end - 2, end, &val11, &val12);
	if(fabs(a[(end - 1) * n + (end - 1)] - val11) < fabs(a[(end - 1) * n + (end - 1)] - val12) ){
			val12 = val11;
	}
	EigValDim2Sdvig(a, n, end - 3, end, &temp_val, &val11);
	if(fabs(a[(end - 1) * n + (end - 1)] - temp_val) < fabs(a[(end - 1) * n + (end - 1)] - val11)){
			val11 = temp_val;
	}
	...
	int flag;
		...
		cout<<possibleValues[0]<<" "<<possibleValues[1]<<" "<<possibleValues[2]<<" "<<possibleValues[3]<<" "<<a[(end - 1) * n + (end - 2)]<<endl;
    for (int k = 2; k < 4 ; k++) {
		s_k = possibleValues[k];
	//	if(k==1 && fabs(val11/val12 - 1) > 0.5) continue;
	...
	cout<<"Nevozmozno deformirofat'"<<endl;
	exit(0);
}
...
//	LOG(Norma);
	...
	double* RL_save = new double[n];
	fill(RL_save, RL_save + n,0);
...
//	return;
	...
	EigenValImpl(a, n, eig, 0, n, Norma, save, RL_save);
...
void EigenValImpl(double* a, int size, double* eig, int begin, int end, double Norma, double* save, double *RL_save) {
	...
	//		printf("Reccurent func with (%d,%d) and (%d,%d)\n",begin,i,i,end);
			EigenValImpl(a, size, eig, begin, i, Norma, save, RL_save);
			EigenValImpl(a, size, eig, i, end, Norma, save, RL_save);
			return;
		}
	}
	...
	while (fabs(a[(end - 1) * size + (end - 2)]) > EPS* Norma) {
		...
		RLmult(a, size, begin, end,RL_save);
		}
	...
	EigenValImpl(a, size, eig, begin, end - 1, Norma, save, RL_save);
}
...
void EigValDim2Sdvig(double* a, int size, int begin, int end, double* val1, double* val2) {
	...
	double det = a[begin * size + begin] * a[(begin + 1) * size + (begin + 1)] - a[(begin + 1) * size + begin] * a[(begin)*size + (begin + 1)];
	double D = trace * trace - 4 * det;
       // double b= -(a[begin * size + begin] + a[(begin + 1)* size + begin  + 1] );
	double LocalNorma = max(fabs(a[begin * size + begin]) + fabs(a[(begin ) * size + begin  + 1 ]), fabs(a[ (begin + 1) * size + begin ]) + fabs(a[(begin + 1) * size + begin + 1]));
	LocalNorma=min(LocalNorma,1.0);
	//LOG(D);
        //(-trace > 0? (trace - sqrt(D))/2 : D/((trace - sqrt(D))/2));
        //(-trace < 0? (trace + sqrt(D))/2 : D/((trace + sqrt(D))/2));
    if (D < 0 && fabs(D)>EPS*LocalNorma) {
		...
		D = fabs(D);
               ...
               *val1 = (trace - sqrt(D)) / 2;
			   ...
			   // cout<<"Po normalnoy formule: "<<*val1<<" "<<*val2<<endl;
                // *val1 = (trace - sqrt(D)) / 2;
                // *val2 = (trace + sqrt(D)) / 2;
				// cout<<"Po plohoi formule: "<<*val1<<" "<<*val2<<endl;
	}
...
void RLmult(double* a, int n, int k1, int k2, double *RL_save) {
	...
	for (int i = k1 + 1; i < k2; i++) { // stroki
		...
		for (int l = i; l < i + krai ; l++) { //stolci
			a[i * n + l ] += a[i * n + (l + 1)] * RL_save[ l ];
		}
		
		for (int l = i + krai; l < k2 - 1; l+=4) { //stolci
			a[i * n + l ]    += a[i * n + (l + 1)] * RL_save[ l ];
			a[i * n + l + 1] += a[i * n + (l + 2)] * RL_save[ l + 1];
			a[i * n + l + 2] += a[i * n + (l + 3)] * RL_save[ l + 2];
			a[i * n + l + 3] += a[i * n + (l + 4)] * RL_save[ l + 3];
		}
